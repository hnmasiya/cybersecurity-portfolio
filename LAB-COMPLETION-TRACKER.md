...
| Attack Simulation & Detection Engineering Lab | In progress — 1 of 6 evidence-backed | 3 MITRE ATT&CK techniques (T1059 Execution, T1053/T1547 Persistence, T1003 Credential Access) × 2 real, self-owned platforms (home-lab Linux host, Azure DC) — T1059 Execution on Linux has a real, captured Wazuh alert (`Evidence/t1059-linux-alert.json`), including a real false-positive found and fixed against Wazuh's own internal health checks; the other 5 combinations have real simulation scripts and detection rules written, ready to run |
